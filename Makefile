...
install:
	uv pip install --python $(shell which python) "omniloader[all]"

dev:
	uv pip install --python $(shell which python) -e ".[all,dev,docs]"
...
type-check:
	uv tool run ty check omniloader --python $(shell which python)
